...
export const ProtocolOwnedLiquidityGraph = () => {
  ...
  return (
    <Chart
      ...
      dataKey={["treasuryNrkDaiPOL"]}
      bulletpointColors={bulletpoints.pol}
      infoTooltipMessage={tooltipInfoMessages.pol}
      headerText="Protocol Owned Liquidity NRK-DAI"
      expandedGraphStrokeColor={theme.palette.graphStrokeColor}
      headerSubText={`${data && trim(data[0].treasuryNrkDaiPOL, 2)}% `}
      stopColor={[["rgba(128, 204, 131, 1)", "rgba(128, 204, 131, 0)"]]}
    />
  );
};

export const NRKStakedGraph = () => {
  const theme = useTheme();
  const { data } = useTreasuryMetrics({ refetchOnMount: false });
  ...
  const staked =
    ...
    data
      .map(metric => ({
        staked: (metric.sNrkCirculatingSupply / metric.nrkCirculatingSupply) * 100,
...
export const APYOverTimeGraph = () => {
  ...
  let apy =
    data &&
    data.map(entry => ({ timestamp: entry.timestamp, apy: Math.pow(parseFloat(entry.percentage) + 1, 365 * 3) * 100 }));
