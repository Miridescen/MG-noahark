...
import { nrk_dai } from "./AllBonds";
import { JsonRpcSigner, StaticJsonRpcProvider } from "@ethersproject/providers";
import { IBaseAsyncThunk } from "src/slices/interfaces";
import { PairContract, RedeemHelper } from "../typechain";

export async function getMarketPrice({ networkID, provider }: IBaseAsyncThunk) {
  const nrk_dai_address = nrk_dai.getAddressForReserve(networkID);
  const pairContract = new ethers.Contract(nrk_dai_address, PairContractABI, provider) as PairContract;
  const reserves = await pairContract.getReserves();
  const reserves0 = Number(reserves[0].toString());
  const reserves1 = Number(reserves[1].toString());
  const marketPrice = reserves0 > reserves1 ? reserves0 / reserves1 : reserves1 / reserves0;
 ...
 */
export async function getTokenPrice(tokenId = "noahark") {
  let resp;
  try {
